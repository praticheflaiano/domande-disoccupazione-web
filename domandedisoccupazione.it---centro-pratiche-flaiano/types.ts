...
export interface UserInputData {
  ...
  // Dati per Calcolo Preventivo (Forecast)
  ...
  // Dati per Analisi Provvedimento (Analysis)
...
export interface MonthlyPayment {
  monthYear: string; // Es: "Gennaio 2025"
  grossAmount: number;
  netAmount: number;
  ...
  isFuture?: boolean; // Usato per l'anticipo
  dateObj?: Date; // Usato per calcoli interni
}
...
export interface NaspiResult {
  ...
  grossMonthlyAmount: number; // Importo base pieno (30 giorni)
  startDate: string;
  endDate: string;
